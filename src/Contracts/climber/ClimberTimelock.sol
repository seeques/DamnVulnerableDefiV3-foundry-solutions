...
contract ClimberTimelock is ClimberTimelockBase {
    ...
    function execute(
        ...
        // @audit external call possible
        // we can try to set up PROPOSER_ROLE for this contract
        // but first we need to update delay to zero so that getOperationState(id) == OperationState.ReadyForExecution
        // and before that it would be wise to transferownership to an attacker since this is the current onwer of proxy
